import { NextRequest, NextResponse } from "next/server";
import { useRouter } from "next/navigation";


export function decodedToken(token: string): any | null {
    try {
        const decoded = JSON.parse(atob(token.split('.')[1]));
        return decoded as any;
    } catch (error) {
        console.error('Token decoding error:', error);
        return null;
    }
}

export async function middleware(request: NextRequest) {
    const { pathname } = request.nextUrl;
    const url = request.nextUrl.clone();

    const token = request.cookies.get('token' as string)?.value;
    // const token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..N6PBt5Dfc_mlCySFQnBruUtGarSWN8IfGmZyPQa2624"

    // Allow access to the signup page without a token
    if (pathname === '/signup') {
        return NextResponse.next();
    }
    
    if(!token && pathname !== '/uas/login'){
        return NextResponse.redirect(`${process.env.NEXT_PUBLIC_FRONTEND_BASE_URL}/uas/login`)
    }

    if (token) {
        
        const user = decodedToken(token as string) as any;
        // console.log(user)

        // if token is expired then redirect to login page
        if (user.exp < Date.now()/1000 && pathname !== '/uas/login') {
            url.pathname = "/uas/login";
            return NextResponse.redirect(new URL(url, request.url));
        }

    }


    return NextResponse.next();
}

export const config = {
    matcher: [
        '/uas/login',
        '/signup',
        '/feed',
        '/in',
        '/jobs',
        '/notifications',
        '/mynetwork',
        '/messaging',
        ],
}

